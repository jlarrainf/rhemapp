...
import { ReadingGroupValidationError } from "@/lib/readingGroups/validation";
...
function handleFailure(error, fallbackMessage) {
	...
	if (!(error instanceof ReadingGroupPersistenceError)) {
		console.error("Reading groups route failed", {
...
async function readJson(request) {
	...
	if (Number.isFinite(contentLength) && contentLength > 8192) {
		throw new ReadingGroupValidationError("La solicitud es demasiado grande");
	}
	try {
...
export async function GET() {
	try {
		...
		return json({ version: 1, groups: await listReadingGroups(supabase, session.user.id) });
	} catch (error) {
		return handleFailure(error, "No se pudieron cargar tus grupos");
	}
}
...
export async function POST(request) {
	try {
		...
		return json(
			{ version: 1, group: await createReadingGroup(supabase, session.user.id, payload?.name) },
			{ status: 201 },
		);
	} catch (error) {
		return handleFailure(error, "No se pudo crear el grupo");
	}
}
