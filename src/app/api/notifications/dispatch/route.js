...
export async function POST(request) {
	if (!isAuthorizedSchedulerRequest(request)) {
		return mobileJson({ error: "Scheduler no autorizado" }, { status: 401 });
	}
	...
	try {
		...
		const { data: preferences, error: preferenceError } = await admin
			...
			.select("user_id, enabled, local_time, timezone")
			.eq("enabled", true);
		if (preferenceError) throw preferenceError;

		const eligiblePreferences = (preferences || []).filter((preference) => shouldDispatchForPreference({
			localTime: normalizeLocalTime(String(preference.local_time || "").slice(0, 5)),
			timezone: normalizeTimezone(preference.timezone),
			now,
		}));
		if (eligiblePreferences.length === 0) {
			return mobileJson({ version: MOBILE_CONTRACT_VERSION, scheduledFor: now.toISOString(), readingKey: null, processed: 0, sent: 0, skipped: 0, failed: 0, revoked: 0 });
		}

		const userIds = eligiblePreferences.map((preference) => preference.user_id);
		const { data: devices, error: deviceError } = await admin
			.from("push_devices")
			.select("id, user_id, platform, token_ciphertext")
			.in("user_id", userIds)
			.eq("platform", NOTIFICATION_PLATFORM)
			.is("revoked_at", null);
		if (deviceError) throw deviceError;

		const eligibleUserIds = new Set(eligiblePreferences.map((preference) => preference.user_id));
		...
		let reading;
		try {
			reading = await resolveNotificationReading(now);
		} catch (error) {
			console.error("Notification reading unavailable", { name: error?.name || "UnknownError", status: error?.status || 503 });
			return mobileJson({ version: MOBILE_CONTRACT_VERSION, scheduledFor: now.toISOString(), readingKey: null, processed: 0, sent: 0, skipped: 0, failed: 0, revoked: 0, skippedReason: "reading_unavailable" });
		}

		const results = await Promise.all(activeDevices.map(async (device) => {
			try {
				return await dispatchToDevice({ supabaseAdmin: admin, device, reading, scheduledFor: now.toISOString() });
			} catch (error) {
		...
		return mobileJson({
			...
			readingKey: `chile:${reading.dateKey}`,
