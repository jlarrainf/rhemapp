...
import { PushProviderError, sendPushNotification } from "./pushProvider.js";
...
export function createNotificationPayload({ reading, baseUrl = getSiteUrl() }) {
	...
	return {
		...
		url: deepLink,
		data: { path: "/daily", date: reading.dateKey, mode: NOTIFICATION_READING_MODE },
	};
}
...
export async function dispatchToDevice({ supabaseAdmin, device, reading, scheduledFor }) {
	...
	const claim = await claimNotificationDelivery({ supabaseAdmin, deviceId: device.id, readingKey, scheduledFor });
	if (!claim?.claimed || !claim.deliveryId) return { status: "skipped", reason: claim?.reason || "already_processed" };
	try {
		const result = await sendPushNotification({
			token: decryptPushToken(device.token_ciphertext),
			payload: createNotificationPayload({ reading }),
			idempotencyKey: `${device.id}:${readingKey}`,
		});
		await completeNotificationDelivery({ supabaseAdmin, deliveryId: claim.deliveryId, providerMessageId: result.messageId });
		return { status: "sent", deliveryId: claim.deliveryId };
	} catch (error) {
		const pushError = error instanceof PushProviderError
			? error
			: new PushProviderError("No se pudo enviar el aviso", { code: "delivery_failed" });
