...
Estado: Validación completa RF-1 a RF-15 y actualización diaria de santos — 2026-09-16
...
| RF-7 | `scripts/sync-daily-readings.mjs`, `scripts/sync-vatican-news-saints.mjs`, `syncState.js`, pruebas de merge/sync | Cumple | La sincronización conserva proveedor, URL, verificación, `fetchedAt`, `reviewedForDate` y estado de intento; el job diario deja el cambio trazado en Git y los errores internos no son públicos. |
...
npm run test:calendar       # 30 pruebas aprobadas
...
5. En `/daily?date=2026-09-16`, comprobar “También mencionados por Vatican News” con los cuatro nombres “Eufemia”, “Víctor III”, “Cornelio” y “Cipriano”; no debe aparecer texto biográfico.
6. Abrir `/daily?date=2026-10-12`: debe aparecer el santo, pero no “Información adicional” ni un enlace Vatican News ni la lista suplementaria.
...
`SPEC CUMPLIDA` — RF-1 a RF-15 validados; la ampliación de santos, la fuente secundaria y la captura diaria de nombres de Vatican News están implementadas con publicación segura, idempotente y evidencia automatizada/manual. La ejecución real del workflow programado queda delegada a GitHub Actions según su horario UTC.
