...
Estado: Validación completa RF-1 a RF-18 y actualización diaria de santos — 2026-09-16
...
| RF-15 | `vaticanNewsSaints.js`, `scripts/sync-vatican-news-saints.mjs`, workflow diario, fixture, `public/data/daily-readings/2026.json`; `npm run test:vatican-saints`, `npm run sync:vatican-saints -- --dry-run`, `npm run validate:daily` | Cumple | La fuente real del 2026-09-16 se extrajo como “Eufemia”, “Víctor III”, “Cornelio” y “Cipriano”; el dry-run repite la ejecución sin cambios. La fecha, estructura, nombres, orden y duplicados se validan; una captura inválida se rechaza y Daily publica solo nombres en la lista separada. El workflow remoto `35136162075` ejecutó el sincronizador y publicó el commit automático `a46a28c`; `/daily?date=2026-09-16` y `/api/readings?date=2026-09-16` respondieron 200 en producción. |
| RF-16 | `src/app/daily/DailyVerseClient.jsx`; `scripts/test-daily-reading.mjs`; build de producción | Cumple | El contexto se presenta en un `<details>` nativo cerrado por defecto, con fiestas/celebraciones, temporada/color y listas separadas de santos y santas. El contenido principal no muestra provenance ni enlaces. |
| RF-17 | `src/lib/readings/saintNames.js`; `scripts/test-daily-reading.mjs` | Cumple | Los nombres conocidos reciben `San` o `Santa` al renderizar; los tratamientos marianos y tratamientos ya presentes se conservan. El dato publicado y su provenance no se reescriben. |
...
- [x] Las fuentes se muestran después del contexto principal y el aviso de ausencia solo aparece cuando ninguna fuente aporta contexto verificable.
...
```bash
npm run test:readings       # 62 pruebas aprobadas
npm run test:calendar       # 32 pruebas aprobadas
...
3. En `/daily?date=2026-09-15`, comprobar que “Contexto litúrgico” aparece cerrado; al abrirlo, aparecen `Nuestra Señora de los Dolores`, `San Nicomedes` y `Santa Catalina de Génova` en listas simples, sin tarjetas, separadores ni enlaces visibles.
...
5. En `/daily?date=2026-09-16`, comprobar que el contexto no muestra “no está disponible” y, al abrirlo, aparecen `Santa Eufemia`, `San Víctor III`, `San Cornelio` y `San Cipriano`, sin texto biográfico.
...
`SPEC CUMPLIDA` — RF-1 a RF-18 validados; el contexto litúrgico colapsable, la presentación con `San`/`Santa`, la separación de fuentes y la disponibilidad agregada entre fuentes están implementados con publicación segura, idempotente y evidencia automatizada/manual. La ejecución remota fue comprobada mediante el workflow de GitHub Actions y queda configurada para repetirse diariamente según su horario UTC.
