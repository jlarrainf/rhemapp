...
| RF-3 | `test-reading-validator.mjs`, `test-daily-validation.mjs`, regresión de `DailyVerseClient` y verificación visual local | PASS | Se muestran primera lectura, salmo, segunda lectura y Evangelio en orden `1 → 2 → 3 → 4`; cuando `title` coincide con `excerpt`, no se repite como encabezado antes de la tarjeta. |
...
| RF-6 | `resolveSundayDateKey`, `GET /api/readings?mode=sunday` y toggle visual | PASS | El modo dominical usa una única regla centralizada, la interfaz persiste `?mode=sunday` y el 2026-09-14 devuelve la celebración del 2026-09-13. |
...
| RF-11 | `BibleTranslationNotice` en Daily, Random y Rosario; regresión de UI; inspección visual/DOM | PASS | La atribución aparece como “Fuente y traducción” en un disclosure cerrado por defecto, sin panel prominente. Al expandirlo identifica `The Holy Bible in Simple Spanish (spabes)`, API.Bible, dominio público CC0 e integridad del texto; no expone la credencial. |

## Comandos ejecutados

- `npm run test:readings` — 33 pruebas, 33 pasadas.
...
- Verificación HTTP en desarrollo local el 2026-09-14 — `/api/readings?mode=sunday` respondió `dateKey: 2026-09-13`, `dateLabel: 13 de septiembre de 2026`, `mode: sunday` y 4 lecturas.
...
- La vista Daily local muestra la etiqueta `PRIMERA LECTURA` y una sola aparición del extracto dentro de la tarjeta; no renderiza el encabezado duplicado.
- La atribución local se muestra como “Fuente y traducción” cerrada por defecto; al expandirla aparecen la traducción consultada, API.Bible, CC0 y la conservación íntegra del texto.
