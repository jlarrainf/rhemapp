...
| RF-4 | T8: `npm run test:mobile` (9/9), API y formulario de preferencias | PASS | Se validan 08:00, IANA, activación, cambio y desactivación. |
| RF-5 | T10/T11: scheduler y dispatch mockeados en `npm run test:mobile` (9/9) | PASS CON LIMITACIÓN | La resolución, ventana local, reintentos e idempotencia están cubiertos; no se ejecutó un envío contra un proveedor push real. |
| RF-6 | T8/T9/T12: validación de preferencias, revocación e implementación Android de permiso denegado | PASS CON LIMITACIÓN | La ruta de denegación limpia el registro del dispositivo. La interacción del diálogo `POST_NOTIFICATIONS` no se ejecutó en esta sesión; el emulador quedó reservado para validar el flujo OAuth. |
| RF-7 | T12: parser de deep links, `AndroidManifest.xml`, `MainActivity.kt`, `PushMessagingService.kt`, `npm run test:mobile` | PASS CON LIMITACIÓN | Se generan y procesan enlaces con `date=YYYY-MM-DD`; no se abrió una notificación real para validar el salto en ejecución. |
...
| RF-9 | T10/T12: `npm run test:readings` (33/33) y `npm run test:mobile` (9/9) | PASS | Se cubren el calendario chileno, el corte del sábado a las 15:00 y la resolución `today` del scheduler/deep link. |
...
La parte web del workspace se publicó en producción mediante el deployment `dpl_77LujpaPTvwfpPKXrPhV56HTECGG`, en estado `READY`, asociado a [rhemapp.com](https://rhemapp.com). Este deploy incluye las mejoras móviles/PWA, navegación de Lectura del día y Rosario, endpoints de autenticación móvil, preferencias y dispatch de notificaciones, registro push y el selector de tema de la Spec 008. La APK Android se compiló e instaló por separado en el emulador.

La compilación de Vercel confirmó la ruta `/api/auth/mobile/google` entre las funciones desplegadas. La comprobación posterior confirmó `400 application/json` para un `POST` sin credencial y `405` para `GET`, confirmando que la ruta existe y responde con el contrato JSON esperado. Vercel reportó `READY`.
...
`SPEC CUMPLIDA` — RF-12 quedó validado con el flujo Google completo en el emulador, el endpoint JSON publicado y la sesión autenticada visible en Rhemapp. Los envíos push reales, pgTAP, la instalación PWA y algunas pruebas físicas siguen requiriendo configuración o dispositivos externos y permanecen documentados como limitaciones.
