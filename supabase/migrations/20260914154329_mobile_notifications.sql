...
create index notification_deliveries_status_idx
  on public.notification_deliveries (status, updated_at asc);
...
for each row execute function private.set_mobile_notification_updated_at();
...
create policy "Users can register their Android devices"
  on public.push_devices
  for insert
  to authenticated
  with check ((select auth.uid()) = user_id and platform = 'android');

create policy "Users can update their Android devices"
  on public.push_devices
  for update
  to authenticated
  using ((select auth.uid()) = user_id)
...
begin
  if p_device_id is null
    or p_reading_key is null
    or char_length(btrim(p_reading_key)) not between 1 and 200
  ...
  where device_id = p_device_id
    and reading_key = btrim(p_reading_key)
  for update;

  if current_delivery.status in ('sent', 'invalid_token') then
    return jsonb_build_object('claimed', false, 'reason', current_delivery.status, 'deliveryId', current_delivery.id);
  end if;

  if current_delivery.status = 'sending'
    and current_delivery.updated_at > now() - interval '10 minutes' then
    return jsonb_build_object('claimed', false, 'reason', 'in_flight', 'deliveryId', current_delivery.id);
  end if;

  if current_delivery.attempts >= p_max_attempts then
    return jsonb_build_object('claimed', false, 'reason', 'attempt_limit', 'deliveryId', current_delivery.id);
  end if;

  update public.notification_deliveries
  set status = 'sending',
      attempts = current_delivery.attempts + 1,
...
begin
  update public.notification_deliveries
  set status = case when p_error_code = 'invalid_token' then 'invalid_token' else 'failed' end,
